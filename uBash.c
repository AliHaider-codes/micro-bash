...
void executePipe(char *buf){ //esegue i comandi di una pipe binaria ( comando1 | comando2 )
	if(strstr(buf,"| |")!=NULL){printf("pipe: empty command \"| |\"\n");return;}
	...
	if(strstr(buf,"<")!=NULL){printf("only the first command could have input redirection\n");return;}
	com = strtok_r(buf, " \n", &buf);//secondo comando
	opt = strcpy(opt, buf);//opzioni secondo comando
	strcpy(pat, "/bin/");
	strcat (pat, com);//path del secondo comando
	if(strcmp(command,"cd")==0 || strcmp(com,"cd")==0){printf("cd: cd must be used alone (without pipe)\n"); return;} 
	pid_t wstat = 0;
	int pipe_arr[2];
	if(pipe(pipe_arr)==-1){perror("Error:"); return;}
	const pid_t p1=fork();
	if(p1==-1){
		perror("fork");
		return;
		...
		if(dup2(pipe_arr[1],1)==-1){perror("dup2");return;}
		if(close(pipe_arr[0])==-1){perror("close");return;}
		if(close(pipe_arr[1])==-1){perror("close");return;}
		...
		if(p2==-1){
			...
			if(dup2(pipe_arr[0],0)==-1){perror("dup2");return;}
			if(close(pipe_arr[0])==-1){perror("close");return;}
			if(close(pipe_arr[1])==-1){perror("close");return;}
			execute(pat,com,opt);
			exit(0);
		}else{
			if(wait(&wstat) == -1){perror("Wait");return;}
			if (!WIFEXITED(wstat)) {
...
void proc(char *command, char *options, char *redir){//lancia comandi non in pipe (esegue redirezioni se richieste)
	if (redir[1]==' '){printf("redirection file not specified\nthere is a ' ' after '>' or '<'\n");return;}
...
void cmd_control(char *buf){//buf contiene il comando da eseguire
	char* command=malloc(buf_size);
	char* options=malloc(buf_size);
	char* redir=malloc(buf_size);

	command = strtok_r(buf, " ", &buf); //metto dentro command il nome del comando
	if(buf[0] == '>' || buf[0] == '<'){//controllo se c'è da fare una redirezione di I/O
	...
	if(strcmp(command,"cd") == 0){
		if(strstr(buf, "|") != NULL) { // cerca la prima occorenza di "|"
		...
		buf = strtok_r(NULL, "\n", &buf);
		if(chdir(buf) == -1) fprintf(stderr, "Error: %s\n" , strerror(errno));
	} else {
...
int main (int argc, char *argv[]){
		...
		if(buf==NULL){fprintf(stderr, "Cannot allocate the buffer\n");return-1;}
	for(;;){
		char* currDir = getcwd(NULL, 0);//store della current directory
		if(currDir == NULL) fprintf(stderr, "Error: %s\n" , strerror(errno));
		else{	usleep(100000);
